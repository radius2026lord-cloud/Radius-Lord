...
function CountryFlag({ country, size = "small" }: { country: ArabCountry; size?: "small" | "large" }) {
  ...
  return (
    <span className={`grid shrink-0 place-items-center overflow-hidden rounded-[5px] bg-[#edf3f8] shadow-sm ring-1 ring-black/[.05] dark:bg-white/[.07] dark:ring-white/[.08] ${size === "large" ? "h-[23px] w-[30px]" : "h-5 w-[26px]"}`} aria-hidden="true">
      <img src={`https://flagcdn.com/${width}x${height}/${country.iso2}.png`} srcSet={`https://flagcdn.com/${width * 2}x${height * 2}/${country.iso2}.png 2x`} width={width} height={height} alt="" className="block h-auto max-h-full w-auto max-w-full object-cover" loading="lazy" />
    </span>
  );
}
      ...
      <div className="group relative">
        ...
        <div className="relative flex h-[52px] origin-center overflow-visible rounded-[17px] border border-[#ccd9e7] bg-white transition-all duration-300 ease-[cubic-bezier(.22,.8,.25,1)] hover:-translate-y-[1px] hover:border-[#9eb6d0] hover:shadow-[0_8px_22px_rgba(20,121,255,.08)] group-focus-within:-translate-y-[2px] group-focus-within:scale-[1.018] group-focus-within:border-[#4c8dff]/90 group-focus-within:shadow-[0_13px_34px_rgba(20,121,255,.17)] group-focus-within:ring-4 group-focus-within:ring-[#1479ff]/10 dark:border-white/[.10] dark:bg-[#211a25] dark:hover:border-white/[.20] dark:hover:bg-[#26202a] dark:group-focus-within:border-[#6aa8ff] dark:group-focus-within:bg-[#211a25] dark:group-focus-within:shadow-[0_14px_36px_rgba(20,121,255,.20)] dark:group-focus-within:ring-[#1479ff]/20" dir="ltr">
          <SelectDropdown
            ...
            align="left"
            renderValue={(item) => <span className="flex min-w-0 items-center justify-center gap-1.5"><CountryFlag country={item} /><span className="truncate">{item.name}</span><span dir="ltr" className="shrink-0">{item.code}</span></span>}
            renderItem={(item, active) => <span className={`flex w-full items-center justify-between px-2.5 py-2 text-[11px] ${active ? "font-bold" : ""}`} dir="rtl"><span className="flex min-w-0 items-center gap-2.5"><CountryFlag country={item} size="large" /><span className="truncate">{item.name}</span></span><span dir="ltr" className="shrink-0 font-semibold">{item.code}</span></span>}
          />

          <div className="relative min-w-0 flex-1" dir="rtl">
            <span className={`pointer-events-none absolute right-2.5 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-[#e8eff6] text-[#0758e9] transition-all duration-200 ease-[cubic-bezier(.22,.8,.25,1)] dark:bg-[#38363c] dark:text-[#8ab5ff] ${hasValue ? "scale-75 opacity-0" : "opacity-100 group-hover:scale-[1.05] group-focus-within:rotate-[4deg] group-focus-within:scale-[1.12] group-focus-within:bg-[#dceaff] group-focus-within:shadow-[0_6px_18px_rgba(20,121,255,.18)] dark:group-focus-within:bg-[#454149]"}`}>
            ...
            <input
              ...
              className={`h-full w-full bg-transparent text-[#17386d] outline-none transition-[font-size,padding] duration-300 placeholder:text-[12px] placeholder:text-slate-400 dark:text-[#f4f1f5] dark:placeholder:text-[#8f8894] ${hasValue ? "px-3" : "pr-[58px] pl-3"}`}
            />
          </div>
        </div>
      </div>
