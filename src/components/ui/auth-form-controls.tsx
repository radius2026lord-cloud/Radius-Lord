  ...
  return (
    <label className={`block ${className}`}>
      ...
      <div className="group relative">
        ...
        <span className={`absolute right-2.5 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-[#e8eff6] text-[#0758e9] transition-all duration-300 ease-[cubic-bezier(.22,.8,.25,1)] group-hover:scale-[1.05] group-focus-within:rotate-[4deg] group-focus-within:scale-[1.12] group-focus-within:bg-[#dceaff] group-focus-within:shadow-[0_6px_18px_rgba(20,121,255,.18)] dark:bg-[#38363c] dark:text-[#8ab5ff] dark:group-focus-within:bg-[#454149] ${emailField && value ? "opacity-0 pointer-events-none" : ""}`}>
          <Icon className="h-5 w-5" />
        </span>
        <input
          type={type} name={name} autoComplete={autoComplete} value={value}
          onChange={(event) => handleChange(event.target.value)} placeholder={placeholder}
          inputMode={emailField ? "email" : undefined} maxLength={emailField ? 254 : undefined}
          pattern={emailField ? "[A-Za-z0-9._%+\\-]+@[A-Za-z0-9.\\-]+\\.[A-Za-z]{2,}" : undefined}
          autoCapitalize={emailField ? "none" : undefined} spellCheck={emailField ? false : undefined}
          lang={emailField ? "en" : undefined} dir={emailField ? "ltr" : undefined}
          className={`relative h-[52px] w-full origin-center rounded-[17px] border border-[#ccd9e7] bg-white text-[13px] text-[#17386d] outline-none transition-all duration-300 ease-[cubic-bezier(.22,.8,.25,1)] placeholder:text-slate-400 hover:-translate-y-[1px] hover:border-[#9eb6d0] hover:shadow-[0_8px_22px_rgba(20,121,255,.08)] focus:-translate-y-[2px] focus:scale-[1.018] focus:border-[#4c8dff]/90 focus:shadow-[0_13px_34px_rgba(20,121,255,.17)] focus:ring-4 focus:ring-[#1479ff]/10 dark:border-white/[.10] dark:bg-[#211a25] dark:text-[#f4f1f5] dark:placeholder:text-[#8f8894] dark:hover:border-white/[.20] dark:hover:bg-[#26202a] dark:focus:border-[#6aa8ff] dark:focus:bg-[#211a25] dark:focus:shadow-[0_14px_36px_rgba(20,121,255,.20)] dark:focus:ring-[#1479ff]/20 sm:text-[14px] ${emailField ? "px-3 font-[LBC]" : "pr-[58px] pl-11"}`}
        />
        {suffix}
      </div>
      <style jsx global>{`
        .auth-signup-content input[name="email"] { direction:ltr!important; text-align:left!important; padding-left:12px!important; padding-right:12px!important; font-family:LBC,Tahoma,Arial,sans-serif!important; font-size:12px!important; font-weight:400!important; letter-spacing:0!important; }
        .auth-signup-content input[name="email"]::placeholder { font-family:LBC,Tahoma,Arial,sans-serif!important; font-size:12px!important; letter-spacing:0!important; }
      `}</style>
    </label>
  );
}
  ...
  const startLocalSubmitAnimation = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (signupRoot && form) {
      ...
      if (hasEmptyRequired || !terms?.checked || passwordsMismatch || hasInvalidField) {
        setLocalStatus("error"); window.setTimeout(() => setLocalStatus("idle"), 560); return;
      }
    }
  ...
  return <>
    ...
    {controlled && effectiveStatus === "success" && <div className="auth-success-overlay fixed inset-0 z-[140] flex items-center justify-center bg-[#dce5ef]/20 backdrop-blur-[2px] dark:bg-[#1d1721]/25" aria-live="polite" aria-label="تم تسجيل الدخول بنجاح">
      <div className="auth-success-card flex min-w-[220px] flex-col items-center rounded-[24px] border border-white/75 bg-white/90 px-8 py-7 shadow-[0_24px_72px_rgba(31,74,132,.20)] backdrop-blur-xl dark:border-white/[.10] dark:bg-[#302e33]/90 dark:shadow-[0_26px_80px_rgba(0,0,0,.34)]">
        ...
        <div className="mt-4 text-sm font-black text-[#102a63] dark:text-[#f4f1f5]">تم تسجيل الدخول بنجاح</div><div className="mt-1 text-[11px] text-slate-500 dark:text-[#b9b3bd]">جارٍ فتح لوحة التحكم...</div>
      </div>
    </div>}
  </>;
}
