...
export function CollectionItemActions({label="إجراءات العنصر",items}:{label?:string;items:CollectionItemAction[]}){
 ...
 const place=()=>{const el=trigger.current;if(!el)return;const r=el.getBoundingClientRect(),menuH=170,menuW=190,gap=7,openUp=window.innerHeight-r.bottom<menuH+16&&r.top>menuH;setPosition({top:openUp?Math.max(8,r.top-menuH-gap):Math.min(window.innerHeight-menuH-8,r.bottom+gap),left:Math.max(8,Math.min(window.innerWidth-menuW-8,r.right-menuW)),openUp})};
 useEffect(()=>{if(!open)return;place();const close=(e:MouseEvent)=>{const n=e.target as Node;if(!root.current?.contains(n)&&!menu.current?.contains(n))setOpen(false)},esc=(e:KeyboardEvent)=>{if(e.key==="Escape")setOpen(false)},reposition=()=>place();document.addEventListener("mousedown",close);document.addEventListener("keydown",esc);window.addEventListener("resize",reposition);window.addEventListener("scroll",reposition,true);return()=>{document.removeEventListener("mousedown",close);document.removeEventListener("keydown",esc);window.removeEventListener("resize",reposition);window.removeEventListener("scroll",reposition,true)}},[open]);
 ...
 return <div ref={root} className="relative inline-flex" onClick={e=>e.stopPropagation()}>
  <button ref={trigger} type="button" aria-label={label} aria-expanded={open} onClick={()=>{if(!open)place();setOpen(v=>!v)}} className={`group/action relative grid h-8 w-8 place-items-center rounded-[10px] border-2 shadow-[0_3px_9px_rgba(58,84,112,.08)] transition-all duration-200 ${open?"border-[#8bb9f0] bg-[#e9f2ff] text-[#0758e9] dark:border-white/20 dark:bg-white/[.08] dark:text-[#d8d2dc]":"border-[#c8d7e6] bg-white/80 text-slate-500 hover:border-[#8bb9f0] hover:bg-[#edf4fb] hover:text-[#0758e9] dark:border-white/[.12] dark:bg-white/[.04] dark:text-slate-300 dark:hover:bg-white/[.08]"}`}><ChevronDown className={`h-4 w-4 transition-transform duration-200 ${open?"rotate-180":""}`}/><span className="rl-tooltip pointer-events-none absolute bottom-[calc(100%+7px)] left-1/2 z-[60] -translate-x-1/2 translate-y-1 whitespace-nowrap rounded-[8px] border border-[#b9cce0] bg-[#17386d] px-2 py-1 text-[10px] font-medium text-white opacity-0 shadow-lg transition-all duration-200 group-hover/action:translate-y-0 group-hover/action:opacity-100 dark:border-white/[.14] dark:bg-[#e7edf5] dark:text-[#173047]">{label}</span></button>
  {dropdown}
 </div>
}
