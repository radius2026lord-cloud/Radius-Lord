  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      {loading ? (
        ...
        <CollectionGrid>
          {filtered.map((customer) => <CustomerCard key={customer.id} customer={customer} selectionMode={gridSelectionMode} selected={selected.has(customer.id)} onToggle={() => toggleCustomer(customer.id)} onOpen={() => router.push(`/Dashboard/customers/${customer.id}`)} onEdit={() => router.push(`/Dashboard/customers/${customer.id}?edit=1`)} />)}
        </CollectionGrid>
      ) : (
        <>
          <CollectionTable>
            ...
            <CollectionTableBody>{filtered.map((customer) => <CustomerRow key={customer.id} customer={customer} selected={selected.has(customer.id)} onToggle={() => toggleCustomer(customer.id)} onOpen={() => router.push(`/Dashboard/customers/${customer.id}`)} onEdit={() => router.push(`/Dashboard/customers/${customer.id}?edit=1`)} />)}</CollectionTableBody>
          </CollectionTable>
...
function CustomerCard({ customer, selectionMode, selected, onToggle, onOpen, onEdit }: { customer: Customer; selectionMode: boolean; selected: boolean; onToggle: () => void; onOpen: () => void; onEdit: () => void }) {
  return <CollectionCard selectionMode={selectionMode} selected={selected} onToggle={onToggle} onOpen={onOpen}>
    <div className="flex items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#e9f2ff] text-[#0758e9] dark:bg-white/[.06] dark:text-[#8ab5ff]"><UserRound className="h-5 w-5" /></div><div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold">{customer.fullName}</div><div className="mt-1 truncate text-[11px] text-slate-500 dark:text-slate-400">@{customer.username || "—"}</div></div><div className="flex shrink-0 items-center gap-2"><Status status={customer.status} />{!selectionMode&&<CollectionItemActions items={[{label:"عرض التفاصيل",icon:Eye,tone:"view",onClick:onOpen},{label:"تعديل",icon:Pencil,tone:"edit",onClick:onEdit},{label:"حذف",icon:Trash2,tone:"danger",danger:true,separatorBefore:true,disabled:true,onClick:()=>{}}]} />}</div></div>
